...
const info = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M256 512c141.4 0 256-114.6 256-256S397.4 0 256 0S0 114.6 0 256S114.6 512 256 512zM216 336h24V272H216c-13.3 0-24-10.7-24-24s10.7-24 24-24h48c13.3 0 24 10.7 24 24v88h8c13.3 0 24 10.7 24 24s-10.7 24-24 24H216c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-144c-17.7 0-32-14.3-32-32s14.3-32 32-32s32 14.3 32 32s-14.3 32-32 32z"/></svg>`;
...
function formValidation(value) {
  if (!value.trim()) {
    ...
    modal.show("Error! Maximum 40 character", "error", error);
    todoForm.reset();
    return false;
  } else {
...
function removeCompletedTodos() {
  if (validateCompletedTodo()) return;
  todos = JSON.parse(localStorage.getItem("savedTodo"));
...
//fucntion to display right modal
function validateCompletedTodo() {
  let result = 0;

  todos.forEach((todo) => (todo.done ? result++ : null));

  if (result == 0) {
    modal.show("Upps.. No completed todo to clear", "info", info);
    return true;
  } else {
    modal.show("Success! Todos has been deleted", "success", check);
  }
}
